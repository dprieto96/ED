...
public:
	...
	*/
    void estremece() {
		...
		if (_numElems <= 2)return;//Si tengo 2 elementos solo hay un par y un impar, no debo mover nada
		
		int pos = 0;
		
		Nodo* actual = _prim;//Nodo que se utiliza para recorrer la lista

		Nodo* par = _prim;
		Nodo* impar = _prim->_sig;//Al ser _nElems >2 => que en este punto hay par e impar

		while (actual!=nullptr) {
			

			if (pos % 2 == 0) {//PAR
				par->_sig = actual->_sig->_sig;
				par = par->_sig;
