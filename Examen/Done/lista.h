...
public:
	...
	*/
	...
	Lista unir_listas( Lista<T>&lista_A,  Lista<T>&Lista_B) {
		
		Lista <T> lista;
		while (!lista_A.esVacia()) {
			lista.pon_final(lista_A.primero());
			lista_A.quita_ppio();
		}
		while (!Lista_B.esVacia()) {
			lista.pon_final(Lista_B.primero());
			Lista_B.quita_ppio();
		}
    ...
    void estremece() {
		...
		Lista <int> par;
		Lista <int> impar;
		Lista <int> ::Iterator iterador = this->begin();

		while (n > 0) {
			if (n % 2 == 0) {
				par.pon_final(iterador.elem());
			}
			else {
				impar.pon_ppio(iterador.elem());
			}
		...
		this->copia(unir_listas(par, impar));
 	}
