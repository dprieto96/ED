	...
	void minimoACima() {
		...
		if (!this->esVacia()) {
			...
			while (!this->esVacia()) {
				...
				else if (min==this->cima()) {
					apariciones++;
				}

				pila_aux.apila(this->cima());
				this->desapila();
			}

			while (!pila_aux.esVacia()) {//Desapila la auxiliar
				if (min==pila_aux.cima() && apariciones==1) {
					pila_aux.desapila();
					apariciones = 0;
					
				}
				else {
					if (min == pila_aux.cima())apariciones--;
					this->apila(pila_aux.cima());
					pila_aux.desapila();
				}
