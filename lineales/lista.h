...
public:
	...
	**/
	void cortaPorK(unsigned int k, Lista<T>& destino) {
		// A IMPLEMENTAR
		if (esVacia() || k > _numElems) return; //Si la lista esta vacia o el elemento a cortar esta fuera
		...
		if (k<2) { //Caso especial en el que debo mover todo los elementos a destino
			destino._prim = _prim;
			destino._ult = _ult;
