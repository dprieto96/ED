	...
	*/
	...
	void estremece() {
		...
		while (pos--) {
			...
			if (pos % 2 && n != _ult) { // Si es impar y no es el ultimo

				// "Extraemos" el nodo
				extrae(n);
				// Como hay que poner los nodos por orden inverso de aparición
				// nos acaban de solucionar la vida. Lo ponemos el último y ya está
				n->_ant = _ult;
				_ult->_sig = n;
				_ult = n;
				_ult->_sig = NULL;
	...
	void estremece() {
		...
		while (recorre != nullptr) {
			...
			if (pos % 2 != 0 && recorre!=_ult) {//IMPAR -> Extraes y los pares los dejas
				recorre->_ant->_sig = recorre->_sig;

				if (recorre->_sig != nullptr)recorre->_sig->_ant = recorre->_ant;//Caso especial, el ultimo sera el apuntado por ult
				else _ult =recorre->_ant;
