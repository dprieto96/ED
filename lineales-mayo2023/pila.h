...
public:
	...
	Pila<T> hanoificar() {
		...
		Pila <int> aux_this;
		if (!this->esVacia()) {
			hanoi.apila(this->cima());
			this->desapila();

			while (!this->esVacia()) {
				if (hanoi.cima() < this->cima()) hanoi.apila(this->cima());
				else aux_this.apila(this->cima());
				this->desapila();
			}

			this->copia(aux_this);

			Pila <int> hanoi_final;
			while (!hanoi.esVacia()) {
				hanoi_final.apila(hanoi.cima());
				hanoi.desapila();
			}
			Pila <int> this_final;
			while (!this->esVacia()) {
				this_final.apila(this->cima());
				this->desapila();
			}
			this->copia(hanoi_final);
			return this_final;
		}
	}
