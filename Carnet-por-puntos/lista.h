...
NOMBRE Y APELLIDOS MIEMBRO 1: Daniel Prieto Remacha
NOMBRE Y APELLIDOS MIEMBRO 2: José María de la Fuente Bahón
*/
...
public:
    ...
    /*
	 ...
	 O(n) -> recorremos toda la lista una unica vez almacenando los valores en dos sublistas que contienen los pares e impares,
	 estos ultimos añadidos por el principio para invertir su orden.
	 Sabiendo que las otras funciones utilizadas como pon_principio,pon_final, libera(), tienen coste menor, justifica que
	 el mayor coste sea el de recorrer la lista de orden n.
	 
	*/
    void estremece() {
		  /* A IMPLEMENTAR */
		if (_numElems <= 2) return; // Si la lista tiene 0, 1 o 2 elementos (incluyendo el número de elementos), no hace nada

		Nodo* actual = _prim; // Comenzamos desde el primer nodo
		Lista<T> listaPares; // Lista para almacenar los elementos pares
		Lista<T> listaImpares; // Lista para almacenar los elementos impares

		int i = 0;
		while (actual != NULL) {
			Nodo* siguiente = actual->_sig; // Guardamos el siguiente nodo antes de modificar los enlaces

			if (i % 2 == 0) {
				// Si el elemento es par, lo agregamos a la lista de pares
				
				listaPares.pon_final(actual->_elem);
				
			}
			else {
				// Si el elemento es impar, lo agregamos a la lista de impares
				
				listaImpares.pon_ppio(actual->_elem);
			}

			// Borramos el nodo actual de la lista original
			//borraElem(actual);
		...
		/*listaPares.imprime();
		cout << endl;
		listaImpares.imprime();
		cout << endl;*/
		
		
		Nodo* n = listaPares._prim;
		...
		/*_prim = listaPares._prim;
		_ult = listaImpares._ult;

		pon_ppio(_prim->_elem);
		pon_final(_ult->_elem);*/

		// Actualizamos el número de elementos
		//_numElems = listaPares._numElems + listaImpares._numElems;
